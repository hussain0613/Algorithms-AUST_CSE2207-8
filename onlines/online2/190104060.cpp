    ...
    while(Q.top().path_count < SIZE ){
        ...
        for_(i){
            if(current.rca_matrix[current.node][i] != inf){
                ...
                if(child.path_count+1 != SIZE) child.rca_matrix[i][root_data.node]; // not going back to root if still in the middle of the journey
                pre_ruduction_processing(child.rca_matrix, root_data.node, current.node, i);

                int reduction_cost = reduce(child.rca_matrix);
                printf("The path so far: ");
                for(int i = 0; i<child.path_count+1; ++i){
                    printf("%d ", child.path[i]);
                    if(i<SIZE) printf("-> ");
                }
                printf("\n");
                printf("reduced matrix for: %d\n", child.node);
                print_mat(child.rca_matrix);
                child.cost = current.cost + current.rca_matrix[current.node][i] + reduction_cost;
